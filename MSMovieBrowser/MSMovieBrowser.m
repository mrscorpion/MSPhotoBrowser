...
@interface MSMovieBrowser () <UIScrollViewDelegate>
...
- (void)commonInit
{
    ...
    if (self.movies.count > 0) {
        [_backgroundView sd_setImageWithURL:[NSURL URLWithString:((MSMovie *)self.movies[0]).coverUrl]];
    }
...
- (void)setupScrollView
{
    ...
    for (MSMovie *movie in self.movies) {
...
{
    [self.backgroundView sd_setImageWithURL:[NSURL URLWithString:((MSMovie *)self.movies[self.currentIndex]).coverUrl]];
